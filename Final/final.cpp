...
int main(int argc, char** argv) {
	...
	cout << "\t    			 Final Project - Team Nguyen The Nhan and Tran Phuoc Dien" << endl;
	cout << "\n  - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -\n" << endl;
...
void parser(int argc, char** argv) {
	...
	char trace_operation[2];							// n is one character long
	...
	while (fgets(trace_line, 1024 , fp)) {
		...
		else operation = atoi(trace_operation); // Chuyển đổi chuỗi trace_operation thành số nguyên
		...
		if (operation == 0 || operation == 1 || operation == 2 || operation == 3 ) {
			cout << "Operation: " << operation << ", Address: " << hex << address  << '\n' << endl; 
			switch (operation) {
			...
			cout <<"Operation: " <<operation <<": Clear all cache"<< '\n' << endl; 
			switch (operation) {
				case 8: clear_cache(); break;
			}
		}else if (operation == 9) {
			cout <<"Operation: " <<operation << ": Print "<< '\n' << endl;
			switch (operation) {
				case 9: print_stats(); break;
			}
		}
		else {
			// Xử lý trường hợp operation không hợp lệ
    ...
    // read cache miss
		...
		if (way >= 0) {									// if we have an empty line, place the read data in it		
			...
			if (mode == 1) {							// Simulating a cache read communication with L2
				cout << "	[Data Read Miss] Read from L2: L1 cache miss, obtain data from L2 " << hex << addr << '\n' << endl;
			}
		}
        ...
        // if dont have any empty way 
			...
			if (way >= 0) {			
                if(L1_data[way][set].State=='D'){
                    if(mode == 1){
                        cout<< "	[Data write back modified line] write to L2: we have a data cache miss: "<< hex << L1_data[way][set].address<< '\n' << endl;
                    }
                }				
...
void clear_cache() {
	...
	cout << "\n\t Clear the cache to the initial state and reset the statistics" << endl << endl;
...
void write(unsigned int addr) {
	...
	if (way >= 0) {		
		// cache write hit							// if we have a matching tag, then we have an data cache hit! 
		statistics.data_hit++;					// Incremenet the data hit counter (We have a new hit!)
		
		if (mode == 1) {							// Simulating an write back communication from L2
				cout << "	[Data Write back] Write to L2: We have Data Cache Hit " << hex << addr << '\n' << endl;
		}	
		L1_data[way][set].State = 'D';		// stay in the modified state
		L1_data[way][set].tag_bits = tag;
		L1_data[way][set].set_bits = set;
		L1_data[way][set].address = addr;
		L1_LRU(way, set, 'D');
		...
		if (mode == 1) {								// Simulating a cache Read For Ownership communication from L2
			cout << "	[Data Write Miss] Read for Ownership from L2 " << hex << addr << '\n' << endl;
		}
		...
		if (way >= 0) {							// if we have an empty line, place the read data in it
			...
			if (mode == 1) {							// Simulating an iniial write through communication from L2
				cout << "	[Data Write Through] Write to L2: we have a data Cache Miss " << hex << addr << '\n' << endl;
			}
		}
			...
			if (way >= 0) {							// if we have a way that's 0, (LRU in the data cache, we use it)
				if (mode == 1) {							// Simulating an write back communication from L2
				cout << "	[Data Write back] Write to L2: we have a data Cache Miss " << hex << L1_data[way][set].address << '\n' << endl;
			}
				L1_data[way][set].State = 'V';	
				L1_data[way][set].tag_bits = tag;
				L1_data[way][set].set_bits = set;
				L1_data[way][set].address = addr;
				L1_LRU(way, set,  'D');	// update the L1 data cache LRU bits
...
void fetch_inst(unsigned int addr) {
    ...
    // read cache miss
		...
		if (way >= 0) {									// if we have an empty line, place the read data in it
			...
			if (mode == 1) {							// Simulating a cache read communication with L2
				cout << " [Instruction Read Miss] Read from L2: L1 cache miss, obtain data from L2 " << hex << addr << '\n' << endl;
			}
		}
...
void print_stats() {
	...
	if (statistics.data_miss == 0) {				// If we don't have any misses, then no operations took place!
		...
		for (int j = 0; j < 16384; ++j) {              //int i = 0; i < 4; ++i
    		for (int i = 0; i < 4; ++i) {
        		if (L1_data[i][j].State != 'I') {
					d=d+1;	
            		cout << "Line valid : " << hex << uppercase << setw(8)<< setfill('0') << L1_data[i][j].address
                		 << "	|	 Way : " << i
						 << "	|	 Set :" << hex <<j
                		 << "	|	 State : " << L1_data[i][j].State 
                		 << "	|	 LRU bit : " <<  L1_data[i][j].LRU_bits 
						 << "	|	 Tag : " << hex << uppercase << setw(3)<<setfill('0') << L1_data[i][j].tag_bits
						 << endl ; 
        			}
	...
	if (statistics.inst_miss == 0) {				// If we don't have any misses, then no operations took place!
		...
		for (int m = 0; m < 16384; ++m) {	//int n = 0; n < 2; ++n
    		for (int n = 0; n < 2; ++n) {
        		if (L1_inst[n][m].State != 'I') {
					++c;
            		cout << " Line valid : " << hex << uppercase << setw(8) << setfill('0') << L1_inst[n][m].address 
                		 << "	|	 Way : " << n
						 << "	|	 Set :" << hex <<m
                		 << "	|	 State : " << L1_inst[n][m].State 
                		 << "	|	 LRU bit : " << L1_inst[n][m].LRU_bits 
						 << "	|	 Tag : " <<  hex << uppercase << setw(3) << setfill('0')<< L1_inst[n][m].tag_bits
						 << endl; 
        			}
    }
}
...
L1_inst[way][set].LRU_bits = 0x2; // Set the current set to MRU 1 (0x1 in Hex)
		break;
	}
}
