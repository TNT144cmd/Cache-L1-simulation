...
if (operation == 0 || operation == 1 || operation == 2 || operation == 3) {
    cout << "Operation: " << operation << ", Address: " << hex << address  << endl; 
    switch (operation) {
    ...
    cout <<"Operation: " <<operation <<": Clear all cache"<< endl; 
    switch (operation) {
        case 8: clear_cache(); break;
    }
}else if (operation == 9) {
    cout <<"Operation: " <<operation << ": Print "<< endl;
    switch (operation) {
        ...
        // if dont have any empty way 
			...
			if (way >= 0) {			
                if(L1_data[way][set].State=='D'){
                    if(mode ==1){
                        cout<< "[Data write back] write to L2: we have a data cache miss: "<< hex <<L1_data[way][set].address<<endl;
                    }
                }				// if we have a way that's 0, (LRU in the data cache, we use it)
		...
		if (way >= 0) {							// if we have an empty line, place the read data in it
			...
			if (mode == 1) {							// Simulating an write back communication from L2
				...
				if (way >= 0) {							// if we have a way that's 0, (LRU in the data cache, we use it)
					L1_data[way][set].State = 'V';	// go to the modified state
					L1_data[way][set].tag_bits = tag;
					L1_data[way][set].set_bits = set;
					L1_data[way][set].address = addr;
...
void invalidate(unsigned int addr) {
	...
	for (int i = 0; i < 4; ++i) {
		if (L1_data[i][set].tag_bits == tag) {
			...
			L1_data[i][set].State = 'I';	// change the MESI Protocol to the Invalid state
