...
void L1_LRU(unsigned int way, unsigned int set, bool empty_flag,char which_cache) {
	...
	case ('D'): 	// We're in the L1_data cache if which_chache is 'D' or 'd'
		...
		// mấy way sau đó cũng empty nên k cần xét. OK !!!!!!!?
		if (empty_flag) {
			for (int i = 0; i < way; ++i) {
		...
		// If a way is NOT empty, we compare LRU bits of the current set with the LRU bits of the other sets
			...
			for (int i = 0; i < 4; ++i) {
				if (L1_data[way][set].LRU_bits <= L1_data[i][set].LRU_bits && L1_data[i][set].LRU_bits > 0){
	...
	case ('I'): 	// We're in the L1_instruction cache if which_chache is 'I' or 'i'
		...
		// If a way is NOT empty, we compare LRU bits of the current set with the LRU bits of the other sets
			...
			for (int i = 0; i < 2; ++i) {
				if (L1_inst[way][set].LRU_bits > L1_inst[i][set].LRU_bits && L1_inst[i][set].LRU_bits > 0) {
					--L1_inst[i][set].LRU_bits;     //no need to do anything, keep the same order
				}
			}
		}
